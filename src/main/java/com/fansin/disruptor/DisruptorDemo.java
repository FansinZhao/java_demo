...
//        Disruptor<LongEvent> disruptor = new Disruptor<LongEvent>(factory,bufferSize,Executors.defaultThreadFactory(), ProducerType.MULTI,new BlockingWaitStrategy());
        ...
        Disruptor<LongEvent> disruptor = new Disruptor<LongEvent>(factory,bufferSize,Executors.defaultThreadFactory(), ProducerType.SINGLE,new BusySpinWaitStrategy());
        /*
        * cpu:双核双线程
        * 1000条
        ...
        * */
        ...
        disruptor.handleEventsWithWorkerPool(new LongEventWokerCustomer(),new LongEventWokerCustomer(),new LongEventWokerCustomer(),new LongEventWokerCustomer());
...
class LongEventCustomer implements EventHandler<LongEvent>{
    ...
    public void onEvent(LongEvent longEvent, long l, boolean b) throws Exception {
        //TODO 业务处理
        System.out.println(Thread.currentThread().getId()+":消费数据:longEvent = " + longEvent.getValue()+" "+l+" "+b);
    }
}
