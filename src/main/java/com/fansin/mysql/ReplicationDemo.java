...
public class ReplicationDemo {
    ...
    static void masterWrite(Connection connection){
        try {
            ...
            connection.createStatement().executeUpdate("INSERT  INTO test.replication_table (name,demo)VALUES (\"" + Thread.currentThread().getName() + "\",\"" + System.currentTimeMillis() + "\")");
            connection.commit();
    ...
    static void slaveRead(Connection connection){
        try {
            ...
            connection.setAutoCommit(false);
            ResultSet resultSet = connection.createStatement().executeQuery("SELECT id,name,demo FROM test.replication_table ");
            connection.commit();
            while (resultSet.next()){
                String row = String.format("%d %s %s",resultSet.getLong("id"),resultSet.getString("name"),resultSet.getString("demo"));
